package src
{
	...
	public class StarlingAnimation extends Sprite 
	{
		...
		private var usingMatrix:Boolean = true;
		
		public function StarlingAnimation():void 
		{
			if (stage) init();
		...
		private function onAnimXMLComplete(e:flash.events.Event):void 
		{
			sheetXML = new XML(e.target.data);
		...
		private function displayAnim():void 
		{
			...
			for (j = 0; j <  sheetArray.length; j++ )
			{
				...
				if (usingMatrix)
				{
					mc = AnimationNodeMatrix(sheetArray[j]);
				}
				else
				{
					mc = AnimationNode(sheetArray[j]);
				}
