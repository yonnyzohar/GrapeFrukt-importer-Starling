package src
{
	...
	public class AnimationNode extends Sprite
	{
		...
		public function parseFrames(lst:XMLList):void
		{
			...
			for (var i:int = 0; i < totalFiles; i++)
			{
				...
				obj.x = lst[i].@x;
				obj.y = lst[i].@y;
				obj.scaleX = lst[i].@scaleX;
				obj.scaleY = lst[i].@scaleY;
				
				if (lst[i].@rotation)
				{
					obj.rotation = lst[i].@rotation;
				}
		...
		private function onLoadImageComplete(e:Event):void
		{
			...
			img.pivotX = _registrationPointX;
			img.pivotY = _registrationPointY;
		...
		public function playAnim(count:int):void
		{
			...
			if (proceed)
			{
				...
				{
					visible = true;
					x = animation[curIndex].x;
					y = animation[curIndex].y;
					scaleX = animation[curIndex].scaleX;
					scaleY = animation[curIndex].scaleY;
					
					if (animation[curIndex].rotation)
					{
						rotation = animation[curIndex].rotation;
					}
